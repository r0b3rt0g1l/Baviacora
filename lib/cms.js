export async function getHeroSlides() {
  ...
  const url = `${baseUrl}/api/municipios/baviacora/hero`;
...
export async function getNoticiasFromCMS() {
  ...
  const url = `${baseUrl}/api/municipios/baviacora/noticias`;
  const controller = new AbortController();
  const timeoutId = setTimeout(() => controller.abort(), 8000);
...
export async function getImagenesFromCMS() {
  ...
  const url = `${baseUrl}/api/municipios/baviacora/imagenes`;
  const controller = new AbortController();
  const timeoutId = setTimeout(() => controller.abort(), 8000);
...
export async function getDocumentosFromCMS(filtros = {}) {
  ...
  const url = `${baseUrl}/api/municipios/baviacora/documentos${qs}`;
...
export async function getSevacFromCMS(filtros = {}) {
  ...
  if (filtros.trimestre) params.set("trimestre", String(filtros.trimestre));
  const qs = params.toString() ? `?${params.toString()}` : "";
  const url = `${baseUrl}/api/municipios/baviacora/sevac${qs}`;
